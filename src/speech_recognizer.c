...
static WAVEFORMATEX wavfmt = DEFAULT_FORMAT;
				...
				struct speech_rec_notifier * notify)
{
	...
	/* read pcm thread */
    errcode = create_record_thread((void*)sr->recorder, &sr->recorder->rec_thread);
    if(errcode) {
		LOGE("record thread error\n");
        goto fail;
	}
...
int sr_start_listening(struct speech_rec *sr)
{
	...
	LOGD(" 1:snd_pcm_state=%d\n", snd_pcm_state((snd_pcm_t *)sr->recorder->wavein_hdl));
...
int sr_stop_listening(struct speech_rec *sr)
{
	...
	if (ret != 0) {
		LOGE("write LAST_SAMPLE failed: %d\n", ret);
	...
	while (sr->rec_stat != MSP_REC_STATUS_COMPLETE) {
		...
		if (MSP_SUCCESS != ret)	{
			LOGE("QISRGetResult failed! error code: %d\n", ret);
	...
	if(sr->session_id != NULL) {
		LOGD("end isr session\n");
		QISRSessionEnd(sr->session_id, "normal end");
	}

	LOGD("over one time isr ...\n\n");

	return 0;
}

//static int *pcm_fp = NULL;
...
int sr_write_audio_data(struct speech_rec *sr, char *data, unsigned int len)
{
	...
	//LOGD("\nupload len=%d, sr->audio_status=%d\n", len, sr->audio_status);
	//LOGD("before upload  sr->ep_stat=%d\n", sr->ep_stat);
	//if(sr->audio_status == 0x01) {
	//	pcm_fp = fopen("pcm_test.pcm", "w+");
	//	LOGD("open\n");
	//}
	//if(pcm_fp) {
	//	LOGD("write\n");
	//	fwrite(data, 1, len, pcm_fp);
	//}
	//if(sr->ep_stat == 3) {
	//	LOGD("close\n");
	//	fclose(pcm_fp);
	...
	//LOGD("after uplaod sr->ep_stat=%d\n\n", sr->ep_stat);
	if (ret) {
		LOGE("QISRAudioWrite error, ret=%d\n", ret);
		end_sr_on_error(sr, ret);
		return ret;
	}
...
void sr_uninit(struct speech_rec * sr)
{
	...
	pthread_cancel(sr->recorder->rec_thread);
	pthread_join(sr->recorder->rec_thread, NULL);
