...
static uint8_t handle_sub_msg_payload(char *msg_payload)
{
	uint32_t ret = 0;
	uint16_t payload_len = 0;
	uint16_t msg_data_len = 0;
	mqtt_msg_protocol_t sub_c_msg;
	...
	sub_msg_device_sn = cJSON_GetObjectItem(sub_json_msg, "device_sn");
	sub_msg_type = cJSON_GetObjectItem(sub_json_msg, "type");
	sub_msg_subtype = cJSON_GetObjectItem(sub_json_msg, "subtype");

	sub_c_msg.device_sn = (uint8_t *)malloc(strlen(sub_msg_device_sn->valuestring)+1);
	memcpy(sub_c_msg.device_sn, sub_msg_device_sn->valuestring, strlen(sub_msg_device_sn->valuestring));
	sub_c_msg.device_sn[strlen(sub_msg_device_sn->valuestring)+1] = '\0';
	LOGD("strlen(sub_msg_device_sn->valuestring)=%d\n", strlen(sub_msg_device_sn->valuestring));
	LOGD("strlen(sub_c_msg.device_sn)=%d\n", strlen(sub_c_msg.device_sn));
	LOGD("sub_c_msg.device_sn[%s]\n", sub_c_msg.device_sn);
	LOGD("sub_msg_device_sn->valuestring[%s]\n", sub_msg_device_sn->valuestring);
	LOGD("sub_msg_device_sn->string[%s]\n", sub_msg_device_sn->string);

	sub_c_msg.type = sub_msg_type->valueint;
	sub_c_msg.subtype = sub_msg_subtype->valueint;

	//LOGD("device_sn[%s]\n", sub_c_msg.device_sn);
	//LOGD("type[%d]\n", sub_c_msg.type);
	//LOGD("subtype[%d]\n", sub_c_msg.subtype);

	if(NULL != sub_json_msg) {
		cJSON_Delete(sub_json_msg);
	}
	free(sub_c_msg.device_sn);
...
static void my_message_callback(struct mosquitto *mosq, void *obj, const struct mosquitto_message *message)
{
	...
	if(cfg->verbose){
		if(message->payloadlen){
			...
			handle_sub_msg_payload(message->payload);
			if(cfg->eol){
				printf("\n");
			}
		}else{
